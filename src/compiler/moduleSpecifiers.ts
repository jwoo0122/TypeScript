...
namespace ts.moduleSpecifiers {
    ...
    export function getModuleSpecifiersWithCacheInfo(
        ...
        const ambient = tryGetModuleNameFromAmbientModule(
            moduleSymbol,
            checker
        );
        if (ambient)
    ...
    function getLocalModuleSpecifier(
        ...
        compilerOptions: CompilerOptions,
        host: ModuleSpecifierResolutionHost,
        importMode: SourceFile["impliedNodeFormat"],
        ...
        const fromPaths =
            ...
            tryGetModuleNameFromPaths(
                relativeToBaseUrl,
                paths,
                getAllowedEndings(ending, compilerOptions, importMode),
                host,
                compilerOptions
            );
        const nonRelative =
    ...
    export function forEachFileNameOfModule<T>(
        ...
        const referenceRedirect = host.isSourceOfProjectReferenceRedirect(
            ...
            : undefined;
        const importedPath = toPath(
            importedFileName,
            cwd,
            getCanonicalFileName
        );
        const redirects =
            host.redirectTargetsMap.get(importedPath) || emptyArray;
    ...
    function tryGetModuleNameFromAmbientModule(
        ...
        const ambientModuleDeclareCandidates = mapDefined(
            ...
            (d) => {
                ...
                const exportSymbol =
                    checker.getSymbolAtLocation(exportAssignment);
                ...
                function getTopNamespace(
                    ...
                    while (
                        namespaceDeclaration.flags & NodeFlags.NestedNamespace
                    ) {
    ...
    function getAllowedEndings(
        ...
        if (
            getEmitModuleResolutionKind(compilerOptions) >=
                ModuleResolutionKind.Node16 &&
            importMode === ModuleKind.ESNext
        ) {
            return [Ending.JsExtension];
        }
        switch (preferredEnding) {
            case Ending.JsExtension:
                return [Ending.JsExtension, Ending.Minimal, Ending.Index];
            case Ending.Index:
                return [Ending.Index, Ending.Minimal, Ending.JsExtension];
            case Ending.Minimal:
                return [Ending.Minimal, Ending.Index, Ending.JsExtension];
            default:
        ...
        for (const key in paths) {
            for (const patternText of paths[key]) {
                ...
                const candidates: {
                    ...
                    ending,
                    value: removeExtensionAndIndexPostFix(
                        relativeToBaseUrl,
                        ending,
                        compilerOptions
                    ),
                ...
                if (indexOfStar !== -1) {
                    ...
                    some(
                        candidates,
                        (c) =>
                            c.ending !== Ending.Minimal && pattern === c.value
                    ) ||
                    some(
                        candidates,
                        (c) =>
                            c.ending === Ending.Minimal &&
                            pattern === c.value &&
                            validateEnding(c)
                    )
                ) {
            ...
            switch (mode) {
                ...
                case MatchingMode.Pattern:
                    ...
                    if (
                        ...
                        return {
                            moduleFileToTry: packageName.replace(
                                "*",
                                starReplacement
                            ),
                        };
    ...
    function tryGetModuleNameAsNodeModule(
        ...
        if (!parts && typeof process.versions.pnp !== "undefined") {
            ...
            if (pnpApi) {
                const fromLocator = pnpApi.findPackageLocator(
                ...
                if (fromLocator && toLocator) {
                    ...
                    if (
                        toLocator.reference ===
                        fromInfo.packageDependencies.get(toLocator.name)
                    ) {
                        ...
                        for (const [
                            name,
                            reference,
                        ] of fromInfo.packageDependencies) {
                            if (Array.isArray(reference)) {
                                if (
                                    reference[0] === toLocator.name &&
                                    reference[1] === toLocator.reference
                                ) {
                                    packageName = name;
                                    break;
                                }
        ...
        if (!packageNameOnly) {
            ...
            while (true) {
                ...
                if (packageRootIndex === -1) {
                    moduleSpecifier = removeExtensionAndIndexPostFix(
                        moduleFileName,
                        preferences.ending,
                        options,
                        host
        ...
        const nodeModulesDirectoryName =
            typeof packageName !== "undefined"
                ? packageName +
                  moduleSpecifier.substring(parts.packageRootIndex)
                : moduleSpecifier.substring(
                      parts.topLevelPackageNameIndex! + 1
                  );

        const packageNameFromPath = getPackageNameFromTypesPackageName(
            nodeModulesDirectoryName
        );
        // For classic resolution, only allow importing from node_modules/@types, not other node_modules
        return getEmitModuleResolutionKind(options) ===
            ModuleResolutionKind.Classic &&
            packageNameFromPath === nodeModulesDirectoryName
            ? undefined
            : packageNameFromPath;
        ...
        function tryDirectoryWithPackageJson(packageRootIndex: number): {
            ...
            if (
                ...
                if (
                    ...
                    const conditions = [
                        "node",
                        importMode === ModuleKind.ESNext ? "import" : "require",
                        "types",
                    ];
                    const fromExports =
                ...
                const versionPaths = packageJsonContent.typesVersions
                    ...
                    : undefined;
                if (versionPaths) {
                    const subModuleName = path.slice(
                        packageRootPath.length + 1
                    );
